...
# my $i = 1;
# chomp $store_db[$i];
# next if $store_db[$i] =~ /^DMR_coordinates_dir/;
#     # print OUT $store_db[$i], "\n";
# my @arr1 = split("\t", $store_db[$i]);
...
#         my $DMR_number_by_sample = $h;
#         print OUT2 "REceived in subroutine: $chr, $start, $end, $gene, $dir, $epiind, $sample_name, $DMR_number_by_sample\n";
#         # print OUT2 "REceived in subroutine: $chr, $start, $end, $gene, $dir, $epiind\n";
#         my $FLAG = 0;
...
#                 my $region = 1;
#                 print OUT2 "1st input to hash: $chr, $start, $end, $gene, $dir, $epiind, $sample_name, $DMR_number_by_sample\n\n";
#                 # print OUT2 "1st input to hash: $chr, $start, $end, $gene, $dir, $epiind\n\n";
#                 $store_db{$chr}{$gene}{$dir}{$region}{"start"} = $start;
#                 $store_db{$chr}{$gene}{$dir}{$region}{"end"} = $end;
#                 $store_db{$chr}{$gene}{$dir}{$region}{"epiind"} = $epiind;
#                 $store_db{$chr}{$gene}{$dir}{$region}{"sample_name"} = $sample_name;
#                 $store_db{$chr}{$gene}{$dir}{$region}{"DMR_number_by_sample"} = $DMR_number_by_sample;
#                 # $store_db{$chr}{$gene}{$dir}{$region}{"dir"} = $dir;
#                 # $store_db{$chr}{$gene}{$dir}{$region}{"region"} = $region_counter;
#             }
#         elsif (defined ($store_db{$chr}{$gene}{$dir}))
#             {
#                 print OUT2 Dumper \%{$store_db{$chr}};
...
#                 {
#                     print OUT2 "region in exists case: $region\n";
#                     # my $region_number = $store_db{$chr}{$gene}{$dir}{$region};
#                     # print OUT2 Dumper \$region;
#                     my ($final_start, $final_end);
#                     my $db_start = $store_db{$chr}{$gene}{$dir}{$region}{"start"};
#                     my $db_end = $store_db{$chr}{$gene}{$dir}{$region}{"end"};
#                     my $db_epiind = $store_db{$chr}{$gene}{$dir}{$region}{"epiind"};
...
#                     my $db_DMR_number_by_sample = $store_db{$chr}{$gene}{$dir}{$region}{"DMR_number_by_sample"};
#                     print OUT2 "inside exists: $chr,$start, $end, $gene,$sample_name, $DMR_number_by_sample,$db_start,$db_end, $region, $db_epiind, $db_sample_name, $db_DMR_number_by_sample\n";
#                     # print OUT2 "inside exists: $chr,$start, $end, $gene,$db_start,$db_end, $region, $db_epiind\n";
...
#                             $final_end = max($end, $db_end);
#                             $FLAG = 0;
#                             print OUT2 "Condition 5\n";
#                         }
#                     }
                    
#                     else {
#                         # No overlap or containment, create a new region
#                         $final_start = $start;
#                         $final_end = $end;
#                         $FLAG = 1;
#                         print OUT2 "Condition 6\n";
#                     }

#                     if ($FLAG == 0)
#                         {
#                             # $store_db{$chr}{$gene}{$dir}{$region} = $region_number;
#                             print OUT2 "here flag=0\n";
#                             $store_db{$chr}{$gene}{$dir}{$region}{"start"} = $final_start;
#                             $store_db{$chr}{$gene}{$dir}{$region}{"end"} = $final_end;
#                             my $new_epind = $db_epiind.";".$epiind;
...
#                         {
#                             my $new_region_number = $region + 1;
#                             print OUT2 $new_region_number,"\n";
#                             print OUT2 "there flag=1\n";
#                             # $store_db{$chr}{$gene}{$dir}{$region} = $region_number;
#                             $store_db{$chr}{$gene}{$dir}{$new_region_number}{"start"} = $final_start;
#                             $store_db{$chr}{$gene}{$dir}{$new_region_number}{"end"} = $final_end;
#                             print OUT2 "$final_start, $final_end \n";
#                             # print OUT2 "$db_epiind, $epiind\n";
#                             # my $new_epind = $db_epiind.";".$epiind;
#                             # $store_db{$chr}{$gene}{$dir}{$new_region_number}{"epiind"} = $new_epind;
#                             # print OUT2 "$db_sample_name, $sample_name\n";
#                             # my $new_sample_name = $db_sample_name.";".$sample_name;
...
#     }
                    ...
                    #         my $new_region_number = $region + 1;
                    #         print OUT2 $new_region_number,"\n";
                    #         print OUT2 "there flag=2\n";
                    #         # $store_db{$chr}{$gene}{$dir}{$region} = $region_number;
                    #         $store_db{$chr}{$gene}{$dir}{$new_region_number}{"start"} = $final_start;
                    #         $store_db{$chr}{$gene}{$dir}{$new_region_number}{"end"} = $final_end;
                    #         # my $new_epind = $db_epiind.";".$epiind;
                    #         $store_db{$chr}{$gene}{$dir}{$new_region_number}{"epiind"} = $epiind;
                    #         # my $new_sample_name = $db_sample_name.";".$sample_name;
                    #         $store_db{$chr}{$gene}{$dir}{$new_region_number}{"sample_name"} = $sample_name;
                    #         # my $new_DMR_number_by_sample = $db_DMR_number_by_sample.":".$DMR_number_by_sample;
                    #         $store_db{$chr}{$gene}{$dir}{$new_region_number}{"DMR_number_by_sample"} = $DMR_number_by_sample;
                    #         print OUT2 "Condition 11\n\n";
                    #     }
...
# if ($db_start == $start && $db_end > $end)
                    ...
                    #         # $store_db{$chr}{$gene}{$dir}{$region}{"region"} = $region_counter;
                    #     }
                    # elsif ($db_start > $start && ($db_start < $end && $db_end > $end ))
                    #     {
                    #         my $db_region_len = $db_end - $db_start + 1;
                    #         # my $current_region_len = $end - $start + 1;
                    #         my $overlap_region_len = $end - $db_start + 1;
                    #         my $overlap = $overlap_region_len/$db_region_len;
                    #         if ($overlap >= 0.5)
                    #             {
                    #                 $final_start = $start;
                    ...
                    #                 $FLAG = 0;
                    #                 print OUT2 "Condition 3\n";
                    #                 # $store_db{$chr}{$gene}{$dir}{$region}{"region"} = $region_counter + 1;
                    #             }
                    #     }
                    # elsif (($db_start < $start && $start < $end) && $db_end < $end )
                    #     {
                    #         my $db_region_len = $db_end - $db_start + 1;
                    #         my $overlap_region_len = $db_end - $start + 1;
                    #         my $overlap = $overlap_region_len/$db_region_len;
                    #         if ($overlap >= 0.5)
                    #             {
                    #                 $final_start = $db_start;
                    ...
                    #             # $store_db{$chr}{$gene}{$dir}{$region}{"region"} = $region_counter + 1;
                    #         }
                    #     elsif ($db_start > $start && $db_start > $end)
                    #         {
                    #             $final_start = $start;
                    #             $final_end = $end;
                    #             $FLAG = 1;
                    #             print OUT2 "Condition 7\n";
                    #             # $store_db{$chr}{$gene}{$dir}{$region}{"region"} = $region_counter + 1;
                    #         }
                    #     elsif ($db_end < $start && $db_end < $end)
                    #         {
                    #             $final_start = $start;
                    #             $final_end = $end;
                    #             $FLAG = 1;
                    #             print OUT2 "Condition 8\n";
                    #             # $store_db{$chr}{$gene}{$dir}{$region}{"region"} = $region_counter + 1;
                    #         }
                    #     elsif ($db_end == $start && $db_end == $end)
                    #         {
                    ...
                    #             $final_end = $end;
                    #             $FLAG = 0;
                    #             print OUT2 "Condition 9\n";
                    #         }




# if (exists ($store_db{$chr}{$gene}{$dir}))
#             {
#                my $db_start = $store_db{$chr}{$gene}{$dir}{"start"};
#                my $db_end = $store_db{$chr}{$gene}{$dir}{"end"};
#                my $region_counter = $store_db{$chr}{$gene}{$dir}{"region"};
#                print OUT2 "$chr,$db_start,$db_end,\n";
#                if ($db_start == $start && $db_end > $end)
#                 {
#                     $store_db{$chr}{$gene}{$dir}{"start"} = $db_start;
#                     $store_db{$chr}{$gene}{$dir}{"end"} = $db_end;
#                     $FLAG = 0;
#                     # $store_db{$chr}{$gene}{$dir}{"region"} = $region_counter;
#                 }
#                elsif ($db_start < $start && $db_end == $end)
#                 {
#                     $store_db{$chr}{$gene}{$dir}{"start"} = $db_start;
...
#                     if ($overlap >= 0.5)
#                         {
#                             $store_db{$chr}{$gene}{$dir}{"start"} = $start;
#                             $store_db{$chr}{$gene}{$dir}{"end"} = $db_end;
#                             $FLAG = 1;
#                             # $store_db{$chr}{$gene}{$dir}{"region"} = $region_counter + 1;
#                         }
#                     # else
#                     #     {
#                     #         # think how to manage multiple DMRs in one gene. 
#                     #         # $store_db{$chr}{$gene}{$dir}{"start"} = $db_start;
#                     #         # $store_db{$chr}{$gene}{$dir}{"end"} = $db_end;
#                     #     }
#                 }
...
#         else
#             {
#                 $store_db{$chr}{$gene}{$dir}{$region_counter}{"start"} = $start;
#                 $store_db{$chr}{$gene}{$dir}{$region_counter}{"end"} = $end;
#                 # $store_db{$chr}{$gene}{$dir}{"dir"} = $dir;
#                 # $store_db{$chr}{$gene}{$dir}{"region"} = $region_counter;
#             }
#         }