...
print "Fix DSS output; Remove rownums\n\n";


my $file1 = "WGBS.granges.sm.DMRcate.p05.betacutoff0.2.hg38.csv";
open IN, $file1 or die;
my @WGBS_DMRcate = <IN>;
my $head1 = $WGBS_DMRcate[0];
# print $head1, "\n";
shift @WGBS_DMRcate; #Remove headers to sort
my @sorted_WGBS_DMRcate = sort {
    (split(',',$a))[0] cmp (split(',',$b))[0] ||
    (split(',',$a))[1] <=> (split(',',$b))[1]
} @WGBS_DMRcate;


my $file2 = "WGBS.dmrs.sm.4n.CHD2.6n.control.sig.0.05.delta.2.txt";
open IN2, $file2 or die;
my @WGBS_DSS = <IN2>;
my $head2 = $WGBS_DSS[0];
shift @WGBS_DSS;
my @sorted_WGBS_DSS = sort {
    (split('\t',$a))[0] cmp (split('\t',$b))[0] ||
    (split('\t',$a))[1] <=> (split('\t',$b))[1]
} @WGBS_DSS;


open OUT, ">WGBS_final_output.txt";
print OUT "DMRcate.chr\tDMRcate.start\tDMRcate.end\tDMRcate.width\tDSS.chr\tDSS.start\tDSS.end\tDSS.width\toverlap.chr\toverlap.start\toverlap.end\toverlap\tgene\tDMRcate.methlyDiff\tDSS.methylDiff\tAvg.methylDiff\tdirection\n";

open OUT2, ">test2.txt";


# print OUT Dumper \@sorted_WGBS_DMRcate;

my $lines = scalar(@sorted_WGBS_DMRcate);
my $lines2 = scalar(@sorted_WGBS_DSS);

for (my $i = 0; $i < $lines - 1 ; $i = $i+1)
    {
        # Get each line from DMRcate file
        chomp $sorted_WGBS_DMRcate[$i];
        next if $sorted_WGBS_DMRcate[$i] =~ /^seqnames/;
         # print OUT $store_db[$i], "\n";
        my @arr1 = split(",", $sorted_WGBS_DMRcate[$i]);
        ...
        my $DMR_region_1 = "$chr_1:$start_1-$end_1";
        ...
        next if ($noCpGs_1 < 2); # Skip if noCpgs is <2
        ...
        my $overlap;
        # Get each line from DSS file
        # if ($sigValue_Fisher < 0.05 && $DMR_region_1_width >= 50 && $noCpGs_1 >= 2) {
        for (my $j = 0; $j < $lines2 ; $j++)
            {
                chomp $sorted_WGBS_DSS[$j];
                next if $sorted_WGBS_DSS[$j] eq "chr";
                my @arr2 = split("\t", $sorted_WGBS_DSS[$j]);
                my $chr_2 = $arr2[0];
                my $start_2 = $arr2[1];
                my $end_2 = $arr2[2];
                my $noCpGs_2 = $arr2[4];
                my $methylDiff_2 = $arr2[7];
                # my $sigValue_pValue = $arr2[10];
                # my $DMR_region_2 = $arr2[14];
                my $DMR_region_2 = "$chr_2:$start_2-$end_2";
                my $DMR_region_2_width = $end_2 - $start_2 + 1;

                next if ($DMR_region_2_width < 50); # Skip if length of dmr is <50
                next if ($noCpGs_2 < 5); # Skip if noCpgs is <5
                next if ($chr_2 eq "chrX" || $chr_2 eq "chrY"); # Skip sex chromosomes
        ...
        my $FLAG = 0;
        # if ($DMR_region_2_width >= 50 && $noCpGs_2 >= 5){
        if (($chr_1 eq $chr_2) && ($direction_1 == $direction_2))
            {
                print OUT2 "inside:", $start_1, "\t", $end_1, "\t", $start_2, "\t", $end_2,"\n";
                ...
                if ($start_1 <= $start_2 && $start_2 < $end_1)
                    {
                        ...
                        print OUT2 "Condition1: $chr_1,$chr_2,$i,$j,$overlap,$FLAG,1\n";
                    }
                elsif ($start_1 >= $start_2 && $end_2 > $start_1)
                    {
                        ...
                        # check_and_store($chr_1, $new_start, $new_end, $gene_1, $direction_1);
                        print OUT2 "Condition2: $chr_1,$chr_2,$i,$j,$overlap,$FLAG,2\n";
                    }
                elsif ($start_1 > $start_2 && $start_1 > $end_2)
                    {
                        # $new_start = $start_2;
                        # $new_end = $end_2;
                        print OUT2 "Condition3: $i,$j,NO OVERLAP,3\n";
                    }
                elsif ($end_1 < $start_2 && $end_1 < $end_2)
                    {
                        # $new_start = $start_2;
                        # $new_end = $end_2;
                        print OUT2 "Condition4: $i,$j,NO OVERLAP,4\n";
                    }
